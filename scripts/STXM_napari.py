...
def read_hdr(file_path):
    ...
    Returns:
        tuple: A tuple containing:
            - xstep (float): The X-axis step size.
            - ystep (float): The Y-axis step size.
...
def load_tiff_stack(data_dir, energy_file, xstep, ystep, viewer=None, flip_vertical=True):
    ...
    Args:
        ...
        flip_vertical (bool, optional): Whether to flip images vertically. Defaults to True.
...
def create_element_map(viewer, image_stack, xstep, ystep, element="C", 
    ...
    Parameters:
        ...
        image_stack (np.ndarray): The 3D image data stack (energy, y, x).
        xstep (float): Step size for the X-axis.
        ystep (float): Step size for the Y-axis.
        element (str, optional): Name of the element for the layer name. Defaults to "C".
        pre_range (tuple, optional): Energy range (eV) before the edge. Defaults to (280, 283).
        post_range (tuple, optional): Energy range (eV) after the edge. Defaults to (297, 300).
    """
...
def create_layers(viewer, xstep, image_layer_name="image_stack"):
    ...
    """
    Create Napari Shapes layer and Points layer for ROI extraction.

    Args:
        viewer (napari.Viewer): The active Napari viewer instance.
        xstep (float): The step size for the X-axis (used for scaling).
        image_layer_name (str, optional): The name of the target image layer. Defaults to "image_stack".
...
def roi_manager(viewer, xstep, image_layer_name="image_stack"):
    ...
    Args:
        ...
        image_layer_name (str, optional): The name of the target image layer in the viewer. 
            Defaults to "image_stack".
...
class ROIButtonsWidget(QWidget):
    def __init__(self, viewer, shapes_layer, image_stack, energies, on_update_callback=None):
        ...
        """
        Custom Qt Widget for plotting ROI spectra and managing ROI controls.

        Args:
            viewer (napari.Viewer): The Napari viewer instance.
            shapes_layer (napari.layers.Shapes): The Napari shapes layer containing ROI data.
            image_stack (np.ndarray): The 3D image data stack (energy, y, x).
            energies (list or np.ndarray): Energy values for the X-axis corresponding to the image stack.
            on_update_callback (callable, optional): A callback function to execute when the plot is updated. 
                Defaults to None .
        """
        super().__init__()
        self.viewer = viewer
    ...
    def generate_roi_dataframe(self):
        ...
        Returns:
            pd.DataFrame: A DataFrame containing the spectral data for all ROIs, including:
                - Energy (float): The energy values for each data point.
                - Mean Intensity (float): The calculated mean intensity values.
                - ROI (str): Labels identifying each ROI (e.g., 'a', 'b', 'c').
        """
        roi_data_list = []
...
def normalize_roi_pre_edge(roi_df, e0=284, pre1=-4, pre2=0, norm1=8, norm2=16, nnorm=1, pre_post_line=False):
    ...
    Note:
    For more details on the normalization algorithm and parameters, 
    please refer to the Larch documentation.
